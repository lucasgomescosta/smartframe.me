...
const Scenes = () => {
    ...
    useEffect(() => {
      if(auth.isAuthReady){
        db.collection('scenes').doc(auth.uid).collection('scenes').onSnapshot(querySnapshot => {
          const docs = []
  ...
  const  createScene = () => {
    const newSceneRef = db.collection('scenes').doc(auth.uid).collection('scenes').doc()
    newSceneRef.set({
      name: sceneName
    })
  ...
  return (
    <div>
      ...
      <div className='mx-auto max-w-lg mb-12'>
        <div className='py-1'>
            <span className='px-1 text-sm text-gray-600'>Scene name</span>
            <input onChange={onChange} name='sceneName' value={sceneName} placeholder='' type='text'
